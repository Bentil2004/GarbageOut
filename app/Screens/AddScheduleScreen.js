...
import LottieView from 'lottie-react-native'; // Ensure Lottie is installed
...
const MyModal = ({ isModalVisible, setIsModalVisible }) => {
  ...
  const handleSubmit = () => {
    Alert.alert('Success', 'Location and name submitted!');
    setIsModalVisible(false);
  };
        ...
        <View style={styles.modalContent}>
          ...
          >
            <Text style={styles.modalButtonText}>
              {isLoading ? 'Locating...' : 'Search Location'}
            </Text>
          </TouchableOpacity>

          {/* Show Lottie Animation while fetching location */}
          {isLoading && (
            <LottieView
              source={require('../assets/LocationSearch.json')} // Ensure correct path
              autoPlay
              loop
              style={styles.lottie}
            />
          ...
          {/* Show Submit Button once location is fetched */}
          {locationFetched && !isLoading && (
            <TouchableOpacity
...
const styles = {
  ...
  modalContent: {
    width: '80%', // Make modal large but not full screen
    backgroundColor: 'white',
    padding: 20,
    borderRadius: 10,
  ...
  modalButton: {
    backgroundColor: '#55A57F', // Green color for buttons
    paddingVertical: 12,
    paddingHorizontal: 25,
    borderRadius: 5,
    marginBottom: 10,
